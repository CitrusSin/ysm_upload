...
async fn request_upload_ticket(mcsm: &MCSManagerConfig) -> Result<UploadTicket> {
    let base_url = mcsm.base_url.trim_end_matches('/');
    let client = reqwest::Client::new();
    let response = client
        .post(format!("{base_url}/api/files/upload"))
        .query(&[
            ("apikey", mcsm.api_key.as_str()),
            ("upload_dir", mcsm.upload_dir.as_str()),
            ("daemonId", mcsm.daemon_id.as_str()),
            ("uuid", mcsm.instance_id.as_str()),
        ])
...
async fn upload_file_to_daemon(
    ...
    let upload_url = build_daemon_upload_url_from_ticket(base_url, ticket)?;
...
fn build_daemon_upload_url_from_ticket(base_url: &str, ticket: &UploadTicket) -> Result<String> {
    let base_url = Url::parse(base_url).context("Invalid MCSManager base URL")?;
